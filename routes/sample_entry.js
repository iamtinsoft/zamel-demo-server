...
router.get("/sample_entry/sample_type/:id", async (req, res) => {
  ...
  db.query(
    ...
    function (err, results) {
      if (results.length > 0) {
        ...
        db.query(
          "INSERT INTO `sample_entry` (hub_facility_id,sptp_id,date_sent,time_sent,sample_type_id,temperature,three_pl_id,sending_facility_id,sample_accepted,reasons_for_rejection,comments,sample_month,sample_year) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",
          [
            req.body.hub_facility_id,
            req.body.sptp_id,
            req.body.date_sent,
